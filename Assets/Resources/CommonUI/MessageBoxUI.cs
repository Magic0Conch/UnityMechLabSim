...
public class MessageBoxUI
{
    ...
    public MessageBoxUI(object val)
    {
        ...
        GameObject root = GameObject.Find("MessageBox");
        if (root == null)
        {
            Debug.LogError("场景中未找到 MessageBox 节点，无法弹窗: " + m_dialogInfo.warnInfo);
            return;
        }
        messageBox = GameObject.Instantiate(Resources.Load("CommonUI/MessageBox")) as GameObject;
        messageBox.transform.SetParent(root.transform, false);
