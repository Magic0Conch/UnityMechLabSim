...
{
    static WWWForm MakeUploadForm(Dictionary<string, string> fields, string path)
    {
    ...
    static string GuessDownloadExtension(byte[] bytes)
    {
        ...
        switch (n)
        {
            ...
            case 80 * 256 + 75:
                {
                    string head = Encoding.ASCII.GetString(bytes, 0, Math.Min(bytes.Length, 1024));
                    if (head.Contains("xl/") || head.Contains("worksheets"))
                        return ".xlsx";
                    return ".zip";
                }
            case 82 * 256 + 97:
        ...
        bool looksText = true;
        int check = Math.Min(bytes.Length, 256);
    ...
    static string ExtensionFromContentDisposition(UnityWebRequest request)
    {
        string cd = request.GetResponseHeader("Content-Disposition");
        if (string.IsNullOrEmpty(cd))
            return null;
        int idx = cd.IndexOf("filename=", StringComparison.OrdinalIgnoreCase);
        if (idx < 0)
            return null;
        string name = cd.Substring(idx + 9).Trim().Trim('"', ';', ' ');
        return Path.GetExtension(name);
    }

    static void ResetSlider(Slider slider)
    {
    ...
    static void SetSliderProgress(Slider slider, float progress)
    {
        if (slider == null) return;
        ...
        {
            Text label = slider.GetComponentInChildren<Text>();
            if (label != null) label.text = (progress * 100f).ToString("F2") + "%";
        }
        catch { }
    }

    static bool UploadRequestFailed(UnityWebRequest request, out string detail)
    {
        detail = request.downloadHandler != null ? request.downloadHandler.text : "";
        if (request.result != UnityWebRequest.Result.Success)
            return true;
        string body = (detail ?? "").Trim();
        if (body.Equals("FAILED", StringComparison.OrdinalIgnoreCase))
            return true;
        if (body.StartsWith("unsupported requesttype", StringComparison.OrdinalIgnoreCase))
            return true;
        return false;
    }

    static void ShowUploadFailed(UnityWebRequest request, string detail)
    {
        string msg = "上传失败";
    ...
    public IEnumerator UpLoadFile(string url, string path, string folderName, Slider sliderProgress,Action action = null)
    {
        ...
        WWWForm form = MakeUploadForm(InfoDic, path);
        ...
        while (!op.isDone)
        {
            if (request.uploadHandler != null)
                SetSliderProgress(sliderProgress, request.uploadHandler.progress);
            yield return null;
        }
        ResetSlider(sliderProgress);
        string detail;
        if (UploadRequestFailed(request, out detail))
...
//        formData.Add(new MultipartFormFileSection("file", bts));
    ...
    public XmlDocument ExcelToXml(string excelFilePath)
    {
        ...
        ISheet sheet = SpreadsheetHelper.LoadFirstSheet(excelFilePath);
    ...
    public IEnumerator DownloadZip(string url, List<IMultipartFormSection> formData, Slider slider, string path, Action action = null)
    {
        UnityWebRequest request = UnityWebRequest.Post(url, formData);
        request.downloadHandler = new DownloadHandlerBuffer();
        yield return request.SendWebRequest();
        ...
        {
            slider.value = 0;
            try { slider.GetComponentInChildren<Text>().text = "0.00%"; } catch { }
            try { slider.transform.parent.gameObject.SetActive(false); } catch { }
        }
        if (request.isHttpError || request.isNetworkError)
        {
            Assets.Message.MessageBox("下载失败，请检查网络\n" + request.error);
        }
        else
        {
            byte[] bytes = request.downloadHandler.data;
            File.WriteAllBytes(path, bytes);
            Assets.Message.MessageBox("下载完成");
            action?.Invoke();
        }
    }
    ...
    public IEnumerator getFileDownAdType(string url, List<IMultipartFormSection> formData, Slider slider, string path, Action action = null)
    {
        ...
        else
        {
            ...
            //Debug.Log("UploadState：" + request.downloadHandler.text);
            byte[] bytes = request.downloadHandler.data;
            string ext = ExtensionFromContentDisposition(request);
            if (string.IsNullOrEmpty(ext))
                ext = GuessDownloadExtension(bytes);
