...
public class TUI : MonoBehaviour
{
    ...
    public void DChange(Dropdown dp)
    {
        ...
        else if(index>10)
        {
            ...
            StartCoroutine(web.DownloadZip(WebData.connectUri, formData, slider, path));
            return;
        }
        else
        {
            ...
            formData.Add(new MultipartFormDataSection(Constant.propname.username, sb.ToString()));
            formData.Add(new MultipartFormDataSection(Constant.propname.folderName, "file" + index));
            StartCoroutine(web.DownloadZip(WebData.connectUri, formData, slider, path));
        }
    ...
    public void UChange(Dropdown dp)
    {
        ...
        else if (index == 7)
        {
            ...
            List<IMultipartFormSection> downForm = new List<IMultipartFormSection>();
            downForm.Add(new MultipartFormDataSection(Constant.propname.requesttype, Constant.WebCommand.DOWNCOMMON.ToString()));
            downForm.Add(new MultipartFormDataSection(Constant.propname.username, "videos"));
            StartCoroutine(web.getFileDownAdType(WebData.connectUri, downForm, slider, path, () => action.Invoke()));
