...
public class FlowPath1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        startExam1.excelFilePath = SpreadsheetHelper.ResolveStreamingAsset("Test1");
        startExam2.excelFilePath = SpreadsheetHelper.ResolveStreamingAsset("Test2");
    ...
    public void b1()
    {
        ...
        UnityAction action = delegate ()
        {
            pdf.FileURL = WebData.ApiBase + "/files/m2";
    ...
    public void b2()
    {
        ...
        UnityAction action = delegate ()
        {
            pdf.FileURL = WebData.ApiBase + "/files/m3";
            pdf.gameObject.SetActive(true);
            startTime = Time.time;
            pdfCode = 3;
            RefreshSchedule();
    ...
    public void b3()//子物体要有Text 注意slider要改名
    {
        ...
        DownloadCommonFile("videos", path, null, action);
        startBtn.onClick.RemoveAllListeners();
    }
    public void b4()
    {
        ...
        startBtn.gameObject.SetActive(true);
        nb = 4;
        description.text = "任务名称：安全操作考试（选做）\n任务内容：进行安全操作考试\n\n任务完成要求：选做，建议达到90分以上\n当前状态：" + infoDic["score1"] + "\n";
        slider.gameObject.SetActive(false);
        ...
        UnityAction action = delegate ()
        {
            ...
            if (!SpreadsheetHelper.StreamingAssetExists("Test1"))
            {
                OnSure os = delegate () { ReloadTest1(); };
        ...
        UnityAction refreshAction = delegate ()
        {
            OnSure os = delegate ()
            {
                SpreadsheetHelper.DeleteStreamingAssetVariants("Test1");
                ReloadTest1();
            };
    ...
    public void b5()
    {
        ...
        startBtn.GetComponentInChildren<Text>().text = "开始任务";
        nb = 5;
        description.text = "任务名称：职业工程素养测试（选做）\n任务内容：进行职业工程素养测试\n\n任务完成要求：选做，建议达到90分以上\n当前状态：" + infoDic["score2"] + "\n";
        slider.gameObject.SetActive(false);
        freshProblemBtn.gameObject.SetActive(true);
        UnityAction action = delegate ()
        {
            ...
            if (!SpreadsheetHelper.StreamingAssetExists("Test2"))
            {
                OnSure os = delegate () { ReloadTest2(); };
                Assets.Message.MessageBox("您还未获取题库，是否重新获取？", 1, os);
                return;
            }
            startExam2.excelFilePath = SpreadsheetHelper.ResolveStreamingAsset("Test2");
        ...
        UnityAction refreshAction = delegate ()
        {
            OnSure os = delegate ()
            {
                SpreadsheetHelper.DeleteStreamingAssetVariants("Test2");
                ReloadTest2();
            };
            Assets.Message.MessageBox("是否更新当前题库？", 1, os);
        };
    ...
    public void b9()
    {
        ...
        description.text = description.text = "任务名称：凸凹模虚拟加工（选做）\n任务内容：上传凸凹模虚拟加工\n\n任务完成要求：选做，上传完成凸凹模虚拟加工\n当前状态：" + ans + "\n";
        slider.gameObject.SetActive(true);
        slider.name = "5";
    ...
    public void b10()
    {
        ...
        description.text = description.text = "任务名称：凸凹模现实加工（选做）\n任务内容：上传凸凹模现实加工\n\n任务完成要求：选做，上传完成凸凹模现实加工\n当前状态：" + ans + "\n";
        slider.gameObject.SetActive(true);
        slider.name = "6";
        //UnityAction action = delegate ()
        //{
    ...
    void UploadSubmit(Slider slider, string folderName)
    {
        UnityAction onClick = delegate ()
        {
            ...
            {
                Assets.Message.MessageBox("未配置服务器地址，请返回登录页设置");
                return;
            }
            string path = openDia();
            if (path == null) return;
            slider.gameObject.SetActive(true);
            StartCoroutine(web.UpLoadFile(WebData.connectUri, path, folderName, slider, RefreshSchedule));
        };
        startBtn.onClick.RemoveAllListeners();
        startBtn.onClick.AddListener(onClick);
    }

    void DownloadCommonFile(string resourceName, string savePath, Slider slider, UnityAction onComplete = null)
    {
        List<IMultipartFormSection> formData = new List<IMultipartFormSection>();
        formData.Add(new MultipartFormDataSection(Constant.propname.requesttype, Constant.WebCommand.DOWNCOMMON.ToString()));
        formData.Add(new MultipartFormDataSection(Constant.propname.username, resourceName));
        Action callback = onComplete != null ? () => onComplete.Invoke() : null;
        StartCoroutine(web.getFileDownAdType(WebData.connectUri, formData, slider, savePath, callback));
    }
    ...
    public void ReloadTest2()
    {
        ...
        string path = Application.streamingAssetsPath + "/Test2";
        DownloadCommonFile("职工素养测试", path, null, action);
    }
    public void ReloadTest1()
    {
        ...
        string path = Application.streamingAssetsPath + "/Test1";
        DownloadCommonFile("实验安全考试", path, null, action);
    }
    public void Model1()
    {
        liucheng.SetActive(false);
